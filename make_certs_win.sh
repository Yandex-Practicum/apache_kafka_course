...
function create_certs() {
    ...
    mkdir "kafka-2"
    mkdir "kafka-3"
    mkdir "kafka-controller-0"
    mkdir "kafka-controller-1"
    mkdir "client_certs"
    ...
    openssl req -new -key kafka-server.key -out kafka-controller-0.csr -subj "//CN=kafka-controller-0"
    openssl req -new -key kafka-server.key -out kafka-controller-1.csr -subj "//CN=kafka-controller-1"
    ...
    openssl x509 -req -in kafka-controller-0.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out kafka-controller-0.crt -days 365
    openssl x509 -req -in kafka-controller-1.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out kafka-controller-1.crt -days 365
    ...
    openssl pkcs12 -export -in kafka-controller-0.crt -inkey kafka-server.key -out kafka-controller-0.p12 -name kafka-controller-0 -password pass:password
    openssl pkcs12 -export -in kafka-controller-1.crt -inkey kafka-server.key -out kafka-controller-1.p12 -name kafka-controller-1 -password pass:password
    ...
    keytool -importkeystore -destkeystore ./kafka-controller-0/kafka.keystore.jks -srckeystore kafka-controller-0.p12 -srcstoretype PKCS12 -alias kafka-controller-0 -storepass password -srcstorepass password
    keytool -importkeystore -destkeystore ./kafka-controller-1/kafka.keystore.jks -srckeystore kafka-controller-1.p12 -srcstoretype PKCS12 -alias kafka-controller-1 -storepass password -srcstorepass password
    ...
    keytool -keystore ./kafka-controller-0/kafka.truststore.jks -alias CARoot -import -file ca.crt -storepass password -noprompt
    keytool -keystore ./kafka-controller-1/kafka.truststore.jks -alias CARoot -import -file ca.crt -storepass password -noprompt
